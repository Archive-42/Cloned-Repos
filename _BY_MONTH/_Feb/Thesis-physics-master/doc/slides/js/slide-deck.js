...
SlideDeck.prototype.onDomLoaded_ = function(e) {
  ...
  this.slideStep = new Array(this.slides.length);
...
SlideDeck.prototype.buildNextItem_ = function() { 
  var after_build_ops = ['fade']
  var build_ops = ['highlight-green', 'highlight-red', 'highlight-yellow']

  var slide = this.slides[this.curSlide_];
  var slideStep = this.slideStep[this.curSlide_];
  this.slideStep[this.curSlide_] = slideStep + 1;
  var toBuildStep = 'build-step-' + slideStep;
  var toBuildList = slide.querySelectorAll('.' + toBuildStep);
  var built = slide.querySelector('.build-current');  

  var default_result = false;  
  var unbuild_class = 'unbuild-step-' + slideStep;
  var unbuild_items = slide.querySelectorAll('.' + unbuild_class);
  for (var j = 0, item; item = unbuild_items[j]; j++) {
    item.classList.remove('built');
    if (item.classList.contains('to-disappear')) {
		item.parentElement.removeChild(item);
    }
    if (item.classList.contains('to-pale')) {
		item.classList.add("highlight-code");
    }
    if (item.classList.contains('to-hide')) {
		item.classList.remove("built");
		item.classList.add("to-appear");
    }
    default_result = true;
  }
  ...
  if (built) {
    ...
    for (var i = 0; i < after_build_ops.length; ++i) {
      var s = after_build_ops[i];
      if (built.classList.contains(s)) {
        built.classList.add('build-' + s);
      }
    }
  }

  if (toBuildList.length == 0) {
    return default_result;
  }

  for (var current = 0; current < toBuildList.length; ++current) {
    var toBuild = toBuildList[current];
  
	toBuild.classList.remove(toBuildStep);
	toBuild.classList.add('build-current');
	toBuild.classList.add('built');
	if (toBuild.classList.contains('to-appear')) {
	  toBuild.classList.remove('to-appear');
	  if (toBuild.classList.contains('unbuild-pale'))
  	    toBuild.classList.add('to-pale');
  	  else if (toBuild.classList.contains('unbuild-hide'))
  	    toBuild.classList.add('to-hide');  	    	  
  	  else
  	    toBuild.classList.add('to-disappear');  	  
	}
  }
...
SlideDeck.prototype.makeBuildLists_ = function () {
  for (var i = this.curSlide_, slide; slide = this.slides[i]; ++i) {
    ...
    for (var j = 0, item; item = items[j]; ++j) {
      if (item.classList) {
        item.classList.add('to-appear');
        item.classList.add('build-step-' + to_build++);
